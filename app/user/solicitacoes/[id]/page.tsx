...
export default function DetalhesSolicitacaoPage({params}: { params: { id: string } }) {
    ...
    const id = params.id;
    const {data: solicitacao, isLoading: isLoadingSolicitacao} = useGetSolicitacaoById(id);
    ...
    return (
        <div className="space-y-6">
            ...
            {!isLoadingSolicitacao && !isLoadingPostos &&
                <Card>
                    {solicitacao &&
                        <CardHeader>
                            <div className="flex items-center gap-4">
            ...
            {(isLoadingSolicitacao || isLoadingPostos) && <FullscreenLoader/>}
        </div>
    )
}
