...
export function ListaEscalas({mes, ano}: ListaEscalasProps) {
    const alocacoesFiltradas = filtrarAlocacoesPorMes(3, 2025, alocacoesMock).filter(
        (a) => a.data.getMonth() + 1 === mes && a.data.getFullYear() === ano && a.guardaVidasId === 1
    )
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <div className="font-medium">
                                            {alocacao.data.toLocaleDateString("pt-BR", {
                                                weekday: "long",
                                                day: "numeric",
                                                month: "long"
                                            })}
                                        ...
                                        <div
                                            className="text-sm text-muted-foreground">{obterNomePosto(alocacao.postoId)}</div>
                                    </div>
